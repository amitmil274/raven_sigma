...
bool Raven_Control::init_ros(int argc, char** argv) 
{
	...
	raven_subscriber   = n.subscribe("ravenstate",1,&Raven_Controller::callback_raven_state,this);
	haptic_subscriber = n.subscribe("haptic_msg",1,&Raven_Controller::callback_haptic_state,this);
	...
	if(ros::isInitialized())
	{
		...
		while(ros::ok())
		{
			...
			switch(theKey)
			{
				...
				case '8':
				{
					LEFT_ARM.set_Center(CURR_RAVEN_STATE.pos);
					RIGHT_ARM.set_Center(CURR_RAVEN_STATE.pos);
...
void* Raven_Control::ros_process(void)
{
	if(ros::isInitialized())
	{
		...
		else
		{
			...
			LEFT_ARM_RAVEN.set_Center(CURR_RAVEN_STATE.pos);
			LEFT_ARM_HAPTIC.set_Center(CURR_RAVEN_STATE.pos);
			RIGHT_ARM_RAVEN.set_Center(CURR_RAVEN_STATE.pos);
			RIGHT_ARM_HAPTIC.set_Center(CURR_RAVEN_STATE.pos);
		...
		while(ros::ok() && RECEIVED_FIRST)
		{
			...
			RIGHT_ARM_HAPTIC.set_Current_Pos(CURR_RAVEN_STATE.pos);
			RIGHT_ARM_HAPTIC.set_Current_Ori(CURR_RAVEN_STATE.ori);

			LEFT_ARM_HAPTIC.set_Current_Pos(CURR_RAVEN_STATE.pos);
			LEFT_ARM_HAPTIC.set_Current_Ori(CURR_RAVEN_STATE.ori);
...
void * Raven_Control::static_gravity_process(void* classRef)
{
...
void * Raven_Control::static_haptic_process(void* classRef)
{
...
void Raven_Control::callback_haptic_state(haptic_device msg) 
{
	...
	for(int i=0; i<2; i++)
	{
		...
		CURR_HAPTIC_STATE.grasp_d[i] = msg.grasp[i];
	}

	for(int i=0; i<6; i++)
	{
		CURR_HAPTIC_STATE.pos[i] = msg.position[i];
	}
	
	for(int i=0; i<18; i++)
	{
		CURR_HAPTIC_STATE.ori[i] = msg.orientation[i];
	}
